...
4. Title: Source-linked daily digest
...
> Save the process we just used as a skill called “daily-digest”. Include sources, output format, validation, and the rule that external actions always require approval.

## 5. Routine

Only after two good skill runs:

> Every weekdays 08:00 in the Bot timezone, run the daily-digest skill. Confirm timezone in Settings → Agent before enabling. Post the result in this conversation. If the source is unavailable, report the failure instead of using old data. Do not take external actions.
