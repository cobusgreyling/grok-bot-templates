...
3. Name: **Cloud Spend**
4. Title: Cloud bill notes, not console edits
...
> Save the process we just used as a skill called “cloud-bill-note”. Include sources, output format, validation, and the rule that external actions always require approval.

## 5. Routine

Only after two good skill runs:

> Every Tuesdays 09:00 in the Bot timezone, run the cloud-bill-note skill. Confirm timezone in Settings → Agent before enabling. Week one stays L1. Post the result in this conversation. If the source is unavailable, report the failure instead of using old data. Do not take external actions.
